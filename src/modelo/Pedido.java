...
public class Pedido {
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			// valorPedido += itens[i].valorDeVenda();
				...
				for (int j = 0; j < itens.length; j++) {
					if(itens[i].equals(outroPedido.itens[j])){
						...
						// sim
							// seleciona
							// altera quantidade
							// considera o menor preco
						// nao, inclui produto na lista
					}
					itens[quantProdutos++] = outroPedido.itens[j]; // adiciona na ultima posição vaga da lista?
				}
			}
		}
		outroPedido.limpar();
